...
% Tests figure deployer exceptions
    ...
    methods (Test)
        ...
        function shouldErrorForInvalidOutputType(testCase)
            DF = FigureDeployer('Figure', matlab.ui.Figure.empty);
            testCase.verifyError(@()getStream(DF, 'OutputType', pi), 'MATLAB:validators:mustBeMember'); 
            testCase.verifyError(@()getStream(DF, 'OutputType', ['Hello'; 'World']), 'MATLAB:validation:IncompatibleSize'); 
